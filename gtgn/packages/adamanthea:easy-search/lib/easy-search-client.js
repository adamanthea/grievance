 ...
 * @api public
 */
EasySearch.search = function (name, searchString, callback, onResult) {
  ...
  if (searcher) {
    if(index.use === "minimongo" && onResult && _.isFunction(onResult)) {
        indexOptions.onResult = onResult;
    }
...
EasySearch.pagination = function(name, step) {
  ...
  if (skip >= 0) {
    ...
    skip = EasySearch.getIndex(name).skip;
  }

  return { skip: skip, limit: conf.limit, step: (skip / conf.limit + 1) };
};
